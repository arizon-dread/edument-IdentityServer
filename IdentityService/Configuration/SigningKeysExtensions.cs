...
namespace IdentityService.Configuration
{
    public static class SigningKeysExtensions
    {
        ...
        public static IIdentityServerBuilder AddProductionSigningCredential(this IIdentityServerBuilder builder, IConfiguration config)
        {
            ...
            var rsaCert = KeyVaultExtensions.LoadCertificate(config, "MyRSACert");
            var p256Cert = KeyVaultExtensions.LoadCertificate(config, "MyP256Cert");
            var p384Cert = KeyVaultExtensions.LoadCertificate(config, "MyP384Cert");
            var p521Cert = KeyVaultExtensions.LoadCertificate(config, "MyP521Cert");
